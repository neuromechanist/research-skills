...
- `transparent-icons` skill — flat scientific icons (brain, neuron, EEG cap, DNA, etc.) through the shared Codex or OpenAI backend, or explicit Atlas Cloud, styled by the theme, keeping the model's native alpha (`--transparency-method auto` skips local removal when the cutout is clean; Pillow threshold and opt-in `rembg` + BiRefNet remain as fallbacks).
...
- For icons: OpenAI API key for the OpenAI Images API, or `codex login` for the Codex CLI fallback (preferred). Set `ATLASCLOUD_API_KEY` only when explicitly selecting the Atlas Cloud backend. Optional: `rembg` + `onnxruntime` for the BiRefNet transparency post-process (one-time ~400 MB model download).
