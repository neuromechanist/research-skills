...
Shape matters as much as size. An evaluation grid is embarrassingly parallel, so N independent single-GPU pods burn the same GPU-hours as one pod and finish in 1/N of the wall-clock time; a multi-node instant cluster is distributed-training infrastructure and the wrong instrument for independent cells. The fan-out reference covers slicing by estimated duration rather than item count (a long-context bin can run 10x a short one), queueing leftover slices onto whichever pods stock allowed, verifying one pod end to end before replicating the launch, monitoring the fleet from one detached loop with an ETA calibrated from an early throughput reading rather than from result silence, reaping each finished pod automatically (fetch, verify locally, only then terminate, since a finished pod bills until something acts), and shrinking the roughly 4-minute per-pod startup to about 90 seconds by baking the evaluation datasets and every dependency group into the image. For the rare run that genuinely needs a cluster, the GPU-selection reference includes a rung-4 pre-flight checklist: provision as an instant cluster rather than hand-wired pods, prove the fabric with an NCCL all-reduce before the real job, make checkpoint-resume mandatory, and rehearse at the smallest shape because debugging bills every node at once.

## Never hit the same pitfall twice

The skill ships a pitfall catalog where every entry was hit on a real, billed pod: a base image whose glibc did not match the prebuilt binaries copied into it, `ssh` sessions that do not inherit Docker `ENV`, REST-created pods that get no injected `ssh` key, `rsync` failing `chown` inside a container and killing a `set -e` launch script, a host pool with a fleet-wide container-start bug, and a fire-and-forget launch that silently no-opped and billed 17 idle minutes. Fleet work adds more, each of which impersonates a different bug: `ssh` inside a loop eats the loop's stdin so only the first pod launches while the rest boot and bill, zsh does not word-split an unquoted variable the way bash does so a generated command chain collapses into one malformed command, and `pkill -f` matches full command lines including the `ssh` session running it, killing that session mid-command and reading as network flakiness. Monitoring adds two that cost judgment rather than GPU-minutes: Python block-buffers stdout when piped so a healthy run's log sits empty for minutes, and a zero-match `grep` forwards exit code 1 through `ssh` so the monitor declares a healthy pod unreachable. Each entry pairs the symptom with the fix, and the templates have the fixes already applied.
