...
│   │   ├── skills/{init-project,update-rules,workflow-reference,epic-dev,pr-review-toolkit,codebase-onboarding,implementation-planning,engineering-loop,debugging,agent-fanout,ci-scaffolding,docker-packaging,security-audit,document-processing}/
...
## Plugins
- **project** (v0.5.0): Project lifecycle toolkit with initialization (now scaffolds `.context/decisions/` for Architecture Decision Records and offers an optional default GitHub label set via `project-init-labels`), rule/config updates, epic/sprint workflow, PR review toolkit, CI/CD scaffolding, Docker packaging, security audit, and document processing. Commands: `/init-project`, `/update-rules`, `/epic-dev`, `/epic-status`, `/release-prep`. Codex-facing skills include `epic-dev` for the `/epic-dev` workflow and `pr-review-toolkit` for code, tests, error handling, comments, types, and simplification review. v0.5.0 adds five core engineering workflow skills distilled from frontier-model session analysis, written so mid-capability models (Claude Opus/Sonnet tiers, GPT-class) can execute the same workflow: `codebase-onboarding` (verified reconnaissance before editing: bootstrap sequence, parallel read-only explorer fan-out, onboarding report contract), `implementation-planning` (two planning registers by stakes, pre-registered decision gates, load-bearing-claim verification, mandatory open-judgment-calls list), `engineering-loop` (single-PR change workflow with per-commit gates, pin-test-first refactors, and a `references/background-jobs.md` detached-job protocol), `debugging` (reproduce-isolate-prove-fix-verify loop, anti-shortcut gates, `references/numerical-debugging.md` for reference-parity work), and `agent-fanout` (roles and worker-tier model routing, full-lifecycle briefing templates, supervision and synthesis protocols, worst-case fleet size computed up front with a hard cap of 40 agents per run).
