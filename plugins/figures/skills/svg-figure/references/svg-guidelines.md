...
- **Font size**: a unitless `font-size` is a length in user units (mm, in the mm-viewBox convention), exactly like `width`/`height`. `validate_fonts.py` folds in the root width/viewBox scale and reports the **physical point size**: `font-size="6"` is 6 mm ≈ 17 pt, while a 6 pt label is `font-size="2.1"` (6 × 25.4/72). An explicit unit is absolute (`font-size="6pt"` is 6 pt). Compare the validator's reported pt against the journal minimum (Nature 5 pt, Science/Cell/PNAS 6 pt); for the geometry checks to read the same size, prefer unitless (user-unit) font sizes over explicit `pt`. Trust `validate_fonts.py` output over in-head math.
