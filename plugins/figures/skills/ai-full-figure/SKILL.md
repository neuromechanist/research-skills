...
Sister skills are invoked by their fully qualified `figures:<skill>` names through the host's skill-invocation mechanism. Keep host-specific command or tool syntax in the host adapter, not in shared skill instructions.
...
Invoke `figures:figure-qa` through the host's skill-invocation mechanism after every generation unless the user passed `no-qa`.
