...
- **Readability (run BEFORE the panel):** one fresh-context reviewer restricted to clarity, no merit scoring. Merit reviewers do not line-edit: a panel can run clean while the first page still contains prose the PI cannot follow on first read. Hunt specifically for: sentences spanning 2-3 typeset lines (break them; one idea per sentence), and-chained run-on clauses, pronoun chains ("it/its/this" with distant or ambiguous referents), sentence-initial And/But/Nor and other rhetorical inversions, terms or names used before being introduced to a panelist outside the PI's subfield, abbreviations used before definition, and claims that concede ground needlessly ("we make no claim..."). Report line-anchored findings with length-neutral rewrites; the document is usually at its page limit. A companion **conciseness hunt** (same fresh-context setup, second pass or second agent) targets the opposite failure: overexplaining concepts the panel already understands, redundant restatement ("in other words", a sentence that only summarizes the previous one), throat-clearing lead-ins, stacked intensifiers, and facts repeated beyond their second appearance. Explaining the domain-unfamiliar is good; explaining the obvious is bloat. Conciseness rewrites must be strictly shorter while preserving every number, citation, and required element; `manuscript:humanizer` carries the full AI-pattern catalog for this hunt.
